...
int ins()
{
    ...
    do{
	   ...
	   for(int i=0;i<n;i++)
       {
           ...
           do{
            fflush(stdin);
            cout<<endl<<"Cognome cliente "<<i+1<<" :"<<endl;
        ...
        do{
            cout<<"Data appuntamento del cliente "<<i+1<<" (espressa da gg/mm/aa):"<<endl;
            fflush(stdin);
            gets(archivio[i].data_app);
            fflush(stdin);
            if(strlen(archivio[i].data_app)<0 || strlen(archivio[i].data_app)>10)
                cout<<"Errore"<<endl;
        }while(strlen(archivio[i].data_app)<0 || strlen(archivio[i].data_app)>10);
        ...
        do{
            ...
            fflush(stdin);
            if(strlen(archivio[i].ora_app)<0 || strlen(archivio[i].ora_app)>5)
                cout<<"Errore"<<endl;
        }while(strlen(archivio[i].ora_app)<0 || strlen(archivio[i].ora_app)>5);
...
void stampa(int n)
{
    ...
    do{
        ...
        if(strlen(data)<0 || strlen(data)>10)
            cout<<"Errore"<<endl;
    }while(strlen(data)<0 || strlen(data)>10);
    ...
    do{
        ...
        gets(ora);
        fflush(stdin);
        if(strlen(ora)<0 || strlen(ora)>5)
            cout<<"Errore"<<endl;
    }while(strlen(ora)<0 || strlen(ora)>5);
...
void stampa2(int n)
{
    cout<<"Ecco gli appuntamenti ordinati per data:"<<endl; //MANCA L'ORDINAMENTO!!!
...
n=ins();
    do{
	   ...
	   cout<<"4-Cancellazione dell'appuntamneto di un cliente dato in input;"<<endl;
	   cout<<"5-Esci;"<<endl;
	   cout<<endl;
	   cin>>r;
